...
Round Robin ka basic idea hai ki incoming requests ko sequential order mein servers ke beech distribute karna — ek ek karke, cyclic manner mein.
...
- ❌ Server load check karna.
- ❌ CPU utilization dekhna.
- ❌ Fast/slow server detect karna.
...
Example:
- Server A → Powerful (16 CPU cores).
- Server B → Weak (2 CPU cores).
...
Step 1️⃣: Client Request Aata Hai
...
Step 2️⃣: Hash Function Apply Hota Hai
```
hash("192.168.1.10") = 84523   (example)
```

Step 3️⃣: Modulo Operation
```
84523 % 3 = 2
```
3 = Total Servers

Step 4️⃣: Backend Selection
...
Same IP ⇒ Same hash ⇒ Same modulo ⇒ Same server
...
Servers:
- A → Processing time = 5 sec.
- B → Processing time = 1 sec.

Traffic high.

Result:
- Server A → Connections pile up.
- Server B → Quickly free.

Least Connections:
- More new requests → Server B.
...
Least Connections Algorithm yaha → A choose karega (wrong decision).
...
Agar sirf last response dekhenge → Wrong decision.
...
- Server A → Bahut powerful.
- Server B → Medium.
- Server C → Weak.

Agar hum sirf Least Connections use karein:
- To Jiske paas sabse kam connections → Usko request jayegi.
...
Weight = Server ki ताकत (capacity)

Example:
- Server A → Weight 5 (strong).
- Server B → Weight 2 (medium).
- Server C → Weight 1 (small).

Higher weight ⇒ Zyada load handle kar sakta hai.
...
- Server A powerful hai → 20 connections normal ho sakta.
- Server C weak hai → 2 bhi heavy ho sakta.

Formula actually bol raha:

👉 “Load ko capacity ke hisaab se compare karo”.
